...
class BayesianGhostBusterAgent(VacuumAgent):
    ...
    # Check if there's a high-probability location based on previous successful actions
        if self.world_model.successful_bust():
            # Assume successful_bust() updates an internal state with the latest high-probability target
            target = self.world_model.get_high_prob_target()  # Placeholder method
            if target:
                return target

        # If no high-probability targets, decide based on other criteria or return None
        return None
    ...
    def bust_strategy(self):
        ...
        # Filtering out already busted and probed coordinates to find unbusted and unprobed
        unbusted = [coord for coord in sorted_coords if coord not in busted_coords]
        unprobed = [coord for coord in sorted_coords if coord not in probed_coords]

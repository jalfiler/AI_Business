...
class GhostProbs:
    ...
    def update_probe(self, probe_report):
        color, coord = probe_report
        total_prob = 0
        for c in self.all_coords:
            distance = abs(coord[0] - c[0]) + abs(coord[1] - c[1])  # Manhattan distance
            prob_update = self.probe_probs.get(distance, {}).get(color, 0)
            self.probs[c] *= prob_update
            total_prob += self.probs[c]

        # Normalize probabilities
        if total_prob > 0:
            for c in self.all_coords:
                self.probs[c] /= total_prob
    ...
    def update_bust(self, bust_report):
        is_ghost, coord = bust_report
        if is_ghost:
            for c in self.all_coords:
                self.probs[c] = 1.0 if c == coord else 0.0
        else:
            self.probs[coord] = 0.0
